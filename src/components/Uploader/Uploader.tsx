  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      while (true) {
        ...
        for (let i = 0; i < parts.length - 1; i++) {
          try {
            ...
            const highlights = [
              {
                value: Number(json.total_spend_galactic),
      ...
      localStorage.setItem(
        'upload-history',
        JSON.stringify({ date: new Date().toISOString(), filename: file.name }),
      );
  ...
  return (
    <div className={styles.uploader}>
      <div className={styles.uploaderTitle}>
        Загрузите <span>csv</span> файл и получите <span>полную информацию</span> о нём за
        сверхнизкое время
      </div>
      <div
        className={`${styles.uploaderField} ${fileUploaded ? styles.uploaderFieldUploaded : ''} ${error ? styles.uploaderFieldError : ''}`}
        onDrop={e => {
          e.preventDefault();
